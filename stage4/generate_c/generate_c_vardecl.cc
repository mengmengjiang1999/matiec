  ...
  private:
    ...
    void update_type_init(symbol_c *symbol /* a spec_init_c, subrange_spec_init_c, etc... */ ) {
      ...
      */
       ...
       */
      if (NULL == matiec::analysis_selected_datatype(analysis_, this->current_var_type_symbol)) {debug_c::print(this->current_var_type_symbol); ERROR;}
      if (get_datatype_info_c::is_array(this->current_var_type_symbol))
        this->current_var_type_symbol = matiec::analysis_selected_datatype(analysis_, this->current_var_type_symbol);
      if (NULL == this->current_var_type_symbol) ERROR;      
    }
    ...
    void print_fb_explicit_initial_values(symbol_c *fbvar_name, symbol_c *init_values_list) {
      ...
      for (int i = 0; i < init_list->n; i++) {
        ...
        if (!get_datatype_info_c::is_ANY_ELEMENTARY(matiec::analysis_selected_datatype(analysis_, init_list_elem->value))) {
