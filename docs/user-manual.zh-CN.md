...
`iec2iec` 会输出原生 `METHOD` 与调用结构，不输出合成兼容函数；parser 之后的
显式 AST 兼容 pass 会从原生方法构造临时降低函数并绑定调用，语义检查和
`iec2c` 继续消费该函数，以保持现有静态派发行为和临时 C ABI。parser 输入保持
用户源码，不再追加方法函数文本，也不再替换方法调用文本。
...
- C++ 嵌入接口支持通过 `Compiler::compile_parallel()` 在同一进程内并行编译；批次中的每个任务必须使用不同的 `CompilationContext` 和独立输出目录，返回结果顺序与输入顺序一致。
