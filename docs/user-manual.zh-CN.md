...
- `iec61131-3:2025-experimental`：面向公开证据所确认的 2025 能力演进。当前基础版本与 `legacy` 接受相同语法，它不是完整或认证符合 IEC 61131-3:2025 的声明。
...
`REF(x)` 取得引用，`DREF(x)` 和后缀 `^` 解引用。`-R` 包含 `-r` 的能力，并额外允许非标准的 `REF_TO ANY`，以及在数组或结构成员中更宽松地使用 `REF_TO`。引用类型初始化仍有限制，不要假定所有第 3 版引用初始化形式都已实现。
