...
| --- | --- | --- | --- | --- |
| AST nodes (`symbol_c` subclasses) | Bison actions in `stage1_2/iec_bison.yy`, Stage 3 producer-local scratch, and a few generator helpers | Parent/child fields are non-owning raw pointers; `parent` and `token` borrow nodes. All flow, constant, datatype, resolution, enumeration, and generator results live in `AnalysisStore` records | Arena destruction invokes virtual destructors | Allocate per-compilation nodes in `AstArena`; node links remain non-owning and all analysis/generator state remains context-owned |
