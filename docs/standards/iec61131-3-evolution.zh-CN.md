...
| `CONFIG-VAR-ACCESS` | VAR_ACCESS | 部分实现 | `iec61131-3:2025-experimental` | 原生层级路径 AST、逐段解析、CSV 导出、顺序编译与 CLI 回归 | `cross-vendor-provisional`；支持配置/资源全局量、资源程序输出、结构字段和常量数组下标 |
...
| `ED4-ASSERT` | `ASSERT(IN : BOOL) : VOID` | 部分实现 | `iec61131-3:2025-experimental` | profile-aware AST library registry、生成 C 与运行回归 | `cross-vendor-provisional`；当前为发布态 no-op 策略 |
...
> Access variables in the experimental profile use a bounded hierarchical alias contract and export metadata to `ACCESS.csv`; direct addresses, dynamic subscripts, and runtime communication binding are not implemented.
...
1. `legacy` 和 `iec61131-3:2025-experimental` Profile 已建立，legacy 默认行为保持不变；
2. 能力 ID、实现状态、证据等级和测试引用已进入可校验目录；
3. UTF-8 源码与字节编码 `STRING` 已成为首个有官方公开证据的第四版实验增量；
4. 引用初始化、命名空间、FB 方法、`VAR_ACCESS` 和 `ASSERT` 已分别建立有边界的实验实现与回归。

## 后续工作顺序

1. 按独立能力继续补齐上述“部分实现”条目，不把临时 MATIEC 规则描述为 IEC 规范；
2. 使用公开的中国国标征求意见材料或其他权威公开材料校正实验行为；
3. 为新增证据建立逐项语法、语义、生成 C、运行时和 legacy 兼容回归；
4. 只有在能够逐条建立可信符合性矩阵后，才评估移除 `experimental` 标签。
