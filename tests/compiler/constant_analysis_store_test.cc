...
int main() {
  ...
  assert(matiec::analysis_constant_value(literal)._int64.get() == 7);
  ...
  assert(matiec::analysis_constant_value(literal)._int64.is_undefined());
  assert(matiec::analysis_constant_value(&transient)._int64.is_undefined());
  return 0;
}
