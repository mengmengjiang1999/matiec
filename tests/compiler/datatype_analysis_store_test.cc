...
int main() {
  ...
  assert(matiec::analysis_datatype_candidates(&transient).empty());
  assert(matiec::analysis_selected_datatype(&transient) == nullptr);
  assert(matiec::analysis_scope(&transient) == nullptr);
  return 0;
}
